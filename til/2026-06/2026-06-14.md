# 2026-06-14 언리얼 멀티플레이(Replication) 디버깅 — 이름 권위·PlayerState 복제 타이밍·게임 리셋
...
- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. 플레이어 이름이 hex ID로 표시되던 문제 — ChangeName 권위](#1-플레이어-이름이-hex-id로-표시되던-문제--changename-권위)
- [2. 접속 순번 안정화 — GameMode JoinCounter](#2-접속-순번-안정화--gamemode-joincounter)
- [3. 클라 위젯이 갱신되지 않던 문제 — PlayerState 복제 타이밍](#3-클라-위젯이-갱신되지-않던-문제--playerstate-복제-타이밍)
- [4. 게임 리셋 미동작 — 호출 경로 없는 죽은 코드](#4-게임-리셋-미동작--호출-경로-없는-죽은-코드)
...
1. **구독 직후 즉시 1회 갱신** — 위젯이 늦게 붙으면 그 사이 일어난 첫 변경(델리게이트 브로드캐스트)을 영영 놓친다. 그래서 구독하자마자 `HandleTriesChanged`/`HandleResultChanged`를 현재 값으로 직접 호출해, 화면을 "지금 상태"로 맞춘다. 이건 3번 문제의 본질과 같다 — Property Replication과 달리 델리게이트(이벤트)는 "그 순간"만 전달되므로, 늦게 합류한 구독자는 직접 현재 값을 끌어와야 한다.
