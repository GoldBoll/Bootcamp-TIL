# 2026-06-26 UE5 TeamCarry — UAT 패키징 오류 수정·Steam 세션 디버깅·Git 롤백·PR 수정·패키징 성공
...
- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. UAT 패키징 빌드 오류 수정](#1-uat-패키징-빌드-오류-수정)
- [2. Steam 멀티플레이어 세션 디버깅](#2-steam-멀티플레이어-세션-디버깅)
- [3. Git 브랜치 롤백 (force push)](#3-git-브랜치-롤백-force-push)
- [4. PR 생성 및 base 브랜치 수정](#4-pr-생성-및-base-브랜치-수정)
- [5. UE5 패키징 빌드 성공](#5-ue5-패키징-빌드-성공)
...
런타임 의미상 에러가 아닌 상황인데 `Error` 레벨을 관용적으로 쓴 게 UAT 판정을 트리거했다. 앞으로 실제 예외 상황이 아니라면 `Warning` 이하를 쓰는 습관이 필요하다.
...
팀원(`bbangjun1`)이 PR 없이 `develop`에 직접 커밋 2개를 머지한 것을 발견했다. 팀 협업 규칙 위반이라 롤백이 필요했다.
...
force push는 히스토리를 재작성하므로 팀원의 로컬 브랜치가 원격과 달라진다. 본인이 develop 위에서 작업 중이었다면 리베이스도 필요하다. 팀 채널에 즉시 공지가 필수.
