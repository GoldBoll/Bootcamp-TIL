# 2026-06-23 팀플 네트워크 토대 설계와 Fab BuildId 불일치, 그리고 협동 운반 합의
...
- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. 내 파트 — 접착제·안전망](#1-내-파트--접착제안전망)
- [2. 네트워크 복제 표준을 먼저 세운 이유](#2-네트워크-복제-표준을-먼저-세운-이유)
- [3. 합산 물리는 순수 함수로 분리](#3-합산-물리는-순수-함수로-분리)
- [4. 가구는 계약만 구현 — 인터페이스로 결합도 낮추기](#4-가구는-계약만-구현--인터페이스로-결합도-낮추기)
- [5. Fab BuildId 불일치 트러블슈팅](#5-fab-buildid-불일치-트러블슈팅)
- [6. 회의 결정사항과 내 설계의 접점](#6-회의-결정사항과-내-설계의-접점)
- [7. 오늘 배운 것 정리](#7-오늘-배운-것-정리)

---

## 오늘 한 일 요약

1. TeamCarry 내 파트 구현계획 수립 → `Network/` 도메인에 **권위 헬퍼·합산 유틸·운반 계약·세션 골격** 4종 작성
2. 팀 공통 **복제 표준 문서**(`net-standard.md`) + 코드리뷰/운용 가이드 + **PIE 테스트 케이스 21종** 정리
3. 에디터 `Missing Modules: Fab` 막힘 → **BuildId 불일치**로 원인 규명(엔진 검증으로 해결)
4. `.idea/`(Rider 개인 설정) `.gitignore` 추가
5. md 문서 제외하고 코드만 커밋 → **PR #2**(base: develop) 발행
6. 저녁 회의 — 스테이지 선택/시점/협동 운반 방식 확정
7. (별개) CS 면접 추가 키워드 복습 파일 정리(i++/++i·무브 시멘틱스·TLB 등)

PR: `github.com/GoldBoll/TeamCarry/pull/2` (`feat/net-session-skeleton` → `develop`)

---

## 1. 내 파트 — 접착제·안전망

팀 5명 전원 프로그래머에 도메인이 갈려 있다(Player/Furniture/Level/Core/UI). 내 역할은 단일 기능이 아니라 **통합·빌드·게임필 + 네트워크/물리 조율**. roles.md 원칙이 핵심이다:

> 네트워크·물리는 단일 소유자가 아니라 **각 도메인에 분산** — 복제 변수는 도메인 담당이 소유하고, 신장식이 표준·세션·교차검증으로 묶는다.

그래서 오늘 작업의 방향은 "내가 가구 로직을 다 짠다"가 아니라 **"각 도메인이 자기 복제 변수를 채워 넣을 표준과 계약을 깐다"**가 됐다.

---

## 2. 네트워크 복제 표준을 먼저 세운 이유

멀티에서 가장 흔한 버그가 "호스트만 보이고 클라엔 안 보임"이다. 변수 추가할 때마다 규칙이 흔들리면 통합 단계에서 터진다. 그래서 코드보다 **표준 9원칙**을 먼저 문서화했다.
...
가구 복제 변수(`bIsGrabbed` 등)는 홍민기 소유라 내가 못 건드린다. 대신 **"이 함수들만 있으면 운반이 굴러간다"**는 계약을 인터페이스로 줬다.
...
기존 `ITCInteractable`(포커스·입력)과 **역할을 나눴다** — 이쪽은 운반 상태·물리 전담. 덕분에 가구 도메인과 네트워크 도메인이 한 파일을 동시에 안 건드린다(팀 규칙: 같은 파일 동시수정 금지).
...
TeamCarry 내 모듈        : 53629095   ✅ 일치 (내 코드 정상 컴파일됨)
...
- 진단 단서: `Binaries/Win64/UnrealEditor.modules`의 `BuildId` vs 엔진 `Build.version`의 `Changelist` vs 플러그인의 `.modules` BuildId를 셋 다 비교
...
여담으로, 커밋 직전 `.idea/`(Rider 개인 설정)가 잡혀서 `.gitignore`에 추가했다. `.vs/`는 이미 무시되고 있었는데 JetBrains 쪽만 빠져 있었다.

---

## 6. 회의 결정사항과 내 설계의 접점

저녁 회의(7:54)에서 나온 결정이 오늘 내 코드와 정확히 맞물렸다.

**확정된 것**
- 스테이지 선택: **후보2** — 이사센터 로비 게시판에서 선택 + 직접 트럭 운전(리썰 컴퍼니 풍)
- 시점: 인게임 **3인칭**(벽·문 보이게), 로비 **1인칭**(물건 찾기), **시점 변환 키** 도입
...
- 빌드 확인 강조 → 오늘 BuildId 트러블슈팅 경험이 팀에 공유할 안전망이 됐다.

남은 검토: 회전·각도 동기화, 플레이어-가구 동시 이동, 트럭 낙하 처리(단순 리스폰).
...
5. **설계가 회의 결정과 맞으면 자신감이 붙는다.** 물리 대신 단순 이동을 택한 근거가 회의의 "velocity 동기화 안 됨" 이슈로 검증됐다. 기술 난이도를 과하게 안 올리는 게 기간 내 완성의 핵심.
