# 2026-06-25 BFS 심화 — 그림·보물섬·숨바꼭질·주난의 난 (4문제·5풀이)

> 오늘은 BFS 한 우물만 깊게 팠다. **연결 요소(1926) → 다중 소스 최장거리(2589) → 1차원 BFS(1697) → 0-1 BFS와 라운드 BFS(14497)** 순으로, 같은 큐 골격에서 "무엇을 누적하고 큐를 어떻게 쓰느냐"만 문제에 맞게 바꾸는 연습이었다. 곁들여 알고리즘 풀이 코드 스타일(입출력 가속·`visited` 거리 인코딩·`memset` 필요 조건·1D vs 2D 확산)을 정리해 굳혔다.

## 목차

- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. 그림 (백준 1926) — BFS 연결 요소](#1-그림-백준-1926--bfs-연결-요소)
- [2. 보물섬 (백준 2589) — 다중 소스 BFS·그래프 지름](#2-보물섬-백준-2589--다중-소스-bfs그래프-지름)
- [3. 숨바꼭질 (백준 1697) — 1차원 BFS](#3-숨바꼭질-백준-1697--1차원-bfs)
- [4. 주난의 난 (백준 14497) — 0-1 BFS vs 큐 2개 라운드 BFS](#4-주난의-난-백준-14497--0-1-bfs-vs-큐-2개-라운드-bfs)
- [코드 스타일·문법 메모](#코드-스타일문법-메모)
...
BFS 4문제(풀이 5개)를 풀며 변형을 익혔다.

1. **1926 그림** — 격자 연결 요소: BFS 한 번 = 그림 하나, 센 칸 수 = 넓이
2. **2589 보물섬** — 모든 육지에서 BFS를 돌려 도달 최대 거리의 최댓값(그래프 지름)
3. **1697 숨바꼭질** — 위치를 정점으로 보는 1차원 BFS, `{x-1, x+1, 2x}` 확산
4. **14497 주난의 난** — "점프=한 겹"을 ① 0-1 BFS(덱) ② 큐 2개 라운드 BFS 두 방식으로
...
육지(L)끼리 상하좌우 이동, **두 육지 사이 최단거리 중 최댓값**을 구한다.
...
- **모든 육지 칸을 시작점으로 BFS** → 각 BFS의 최대 거리 중 전체 최댓값이 답(그래프 지름).
...
- **전역 배열은 자동 0**이고 BFS를 한 번만 부르니 `memset` 불필요 → 군더더기 제거. (지역 배열이라면 쓰레기값 때문에 초기화 필수)

## 4. 주난의 난 (백준 14497) — 0-1 BFS vs 큐 2개 라운드 BFS

점프 한 번이 친구(1) **한 겹**을 쓰러뜨린다. 빈칸(0)은 같은 점프로 퍼지고 친구를 넘으면 점프 1번 소모 → **가중치 0/1 최단경로**.
...
- INF를 `0x3f`로 두는 건 작위적이라 **`-1`을 "미방문=무한대"** 로 쓰고 `== -1` 가드로 비교(`memset(dist, -1, …)`).

### 방식 B — 큐 2개 라운드 BFS (튜터님 코드)
...
- `q`를 다 비우면 `cnt++` 하고 `q = temp` → 한 점프 진행. 범인 칸을 쓰러뜨려 `'0'`이 되면 루프 종료.
- `visited`에 **밟은 라운드 번호**를 기록하므로 `visited[목표]`가 곧 답.
- 덱 버전(A)과 결과는 같지만, 큐 2개(B)가 문제의 "한 겹씩 쓰러뜨린다"는 의미와 더 직관적으로 맞는다.

## 코드 스타일·문법 메모

오늘 풀이를 일관되게 맞추며 굳힌 규칙들.

- **입출력 가속 3줄**을 `main()` 첫 줄(어떤 cin/cout보다 먼저, 한 번만)에 둔다.
  ```cpp
  ios_base::sync_with_stdio(false);
  cin.tie(NULL);
  cout.tie(NULL);
  ```
- **`visited` 거리 인코딩**: 시작을 1로, 출력 때 -1. 0(미방문)과 거리 0의 충돌 회피.
- **`memset` 필요 조건**: BFS를 *여러 번* 부르면(다중 소스) 매 호출 리셋 필요. *한 번*이고 전역 배열이면 자동 0이라 불필요. (지역 배열은 쓰레기값이라 항상 초기화)
- **1D는 `for (int x : {a,b,c})`, 2D는 `dy/dx`** 로 확산 후보를 표현.
- **`cnt`(개수) vs `dist`(거리)** — 세는 문제(넓이)는 칸마다 `cnt++`, 거리 문제는 `dist[next]=dist[cur]+1`.
- **0-1 BFS**: 가중치 0/1이면 덱으로 `push_front`/`push_back`. INF는 `-1` 사용.
- **함수 vs 인라인**: BFS를 여러 번 부르면 `bfs(sy,sx)` 함수로, 한 번이면 `main`에 인라인.
- **주석은 파일 맨 위 헤더로 분리** — 본문은 복붙 가능하게 깨끗이.

## 오늘 배운 것 정리

1. **같은 BFS 골격, 다른 누적값** — `while(q)→front/pop→4방향`은 고정이고, 넓이는 `cnt`, 거리는 `dist`, 가중치는 0-1 BFS로 가지를 친다.
2. **그래프 지름은 다중 소스 BFS** — 모든 시작점에서 BFS를 돌려 최대 거리의 최댓값(2589). 이때 거리표를 매 호출 `memset`해야 한다.
3. **`memset`은 "쓰레기값"이 아니라 "재호출 흔적" 때문** — 전역 배열은 자동 0이라 단일 호출이면 불필요. 다중 호출일 때만 직전 거리를 지우려고 필요.
...
6. **0-1 BFS = 가중치 0/1 다익스트라의 덱 버전** — `push_front`(0)/`push_back`(1)으로 우선순위 큐 없이 O(V+E). 범인(`#`)도 사람이라 비용 1.
