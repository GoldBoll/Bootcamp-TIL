...
UE5 C++ 게임 클라이언트 개발자를 지망합니다.  
버그 하나를 잡아도 증상 → 분석 → 해결 순서로 과정을 남긴다는 원칙으로, 팀프로젝트 트러블슈팅과 CS 면접 준비, 알고리즘 풀이를 매일 기록합니다.  
대표 작업은 바로 아래 '핵심 작업 한눈에'에, 전체 과정은 카테고리별 기록에 있습니다.
...
| 카테고리 | 내용 |
|---|---|
| **언리얼** | UE5 C++ — 과제·강의 정리. 하위 `쿠펭`은 협동 이사 게임(TeamCarry), `Baam`은 뱅! 카드게임, `팀프로젝트`는 Ch3 팀플 기록 |
