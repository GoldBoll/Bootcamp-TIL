# 2026-06-10 모의면접 std::map vs TMap 답변 보강 · CS 38 Replication 준비 · 코드카타 바탕화면 정리

> 오늘은 CS 트랙이 두 갈래로 움직였다. 하나는 어제 작성한 **CS 37 "std::map vs TMap"** 파일을 **모의면접 답변용으로 보강**한 것 — 자료구조 차이(RBT 균형 트리 vs 해시 테이블)·복잡도·순회 순서·메모리 레이아웃·RBT 5대 불변식 다섯 축으로 답변 라인을 다듬었다. 다른 하나는 **내일 모의면접 주제 "언리얼 엔진 Replication"(CS 38)** 준비 파일 생성 — 어제 수강한 멀티플레이 강의 챕터 5~7이 그대로 토대가 됐다. 마지막으로 **코드카타 프로그래머스 161990 "바탕화면 정리"(CodeKata_23)** 를 단일 스캔 경계 사각형으로 풀었다. 관통 주제는 컨테이너(37) → 동기화(38)로 이어지는 **언리얼 핵심 시스템 확장**이다.

## 목차

- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. CS 37 모의면접 답변 보강 — std::map vs TMap (RBT)](#1-cs-37-모의면접-답변-보강--stdmap-vs-tmap-rbt)
- [2. 레드블랙 트리가 std::map의 O(log n)을 보장하는 방식](#2-레드블랙-트리가-stdmap의-olog-n을-보장하는-방식)
- [3. TMap은 std::map보다 std::unordered_map에 가깝다](#3-tmap은-stdmap보다-stdunordered_map에-가깝다)
- [4. CS 38 준비 — 언리얼 Replication (내일 주제)](#4-cs-38-준비--언리얼-replication-내일-주제)
- [5. Property Replication과 RPC의 분업](#5-property-replication과-rpc의-분업)
- [6. RepNotify 서버 미호출 함정과 Role](#6-repnotify-서버-미호출-함정과-role)
- [7. 코드카타 161990 바탕화면 정리 (CodeKata_23)](#7-코드카타-161990-바탕화면-정리-codekata_23)
...
1. **CS 37 모의면접 답변 보강** — `raw/cs-notion/37_stdmap_vs_tmap.md`. std::map = 레드블랙 트리(정렬·O(log n) 최악 보장), TMap = 해시 테이블(TSet 기반·평균 O(1)·GC/리플렉션 연동). 다섯 축(자료구조·복잡도·순회 순서·메모리 레이아웃·RBT 불변식)으로 답변 라인 정리.
...
4. **CS 38 "언리얼 Replication" 준비 파일 생성** — `raw/cs-notion/38_unreal_replication.md`. 서버 권위 단방향 복제, Property Replication 3종 세트(bReplicates·UPROPERTY(Replicated)·DOREPLIFETIME), RPC 3종(Server/Client/NetMulticast)·Reliable·WithValidation, Role 3종, 꼬리질문 13개.
...
어제 작성한 37번 파일(`raw/cs-notion/37_stdmap_vs_tmap.md`)을 오늘 모의면접에서 실제로 말할 수 있게 답변 라인으로 다듬었다. 핵심은 **"둘 다 맵이고요…"로 나열하지 않고, 트리/해시라는 내부 구조가 정렬·복잡도·통합까지 어떻게 연쇄로 갈라놓는지"** 를 한 흐름으로 푸는 것이다.
...
## 4. CS 38 준비 — 언리얼 Replication (내일 주제)

내일 모의면접 주제로 CS 38 "언리얼 엔진 Replication" 파일을 만들었다(`raw/cs-notion/38_unreal_replication.md`). 컨테이너·자료구조(37)에서 **네트워크·동기화** 트랙으로 전환하는 지점이고, 어제 수강한 멀티플레이 강의 ch5~7이 그대로 토대다.
