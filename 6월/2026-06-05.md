# 2026-06-05 언리얼 멀티플레이 기초 — 서버 구조·NetMode·NetRole (멀티플레이 강의 챕터 1)
...
- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. 채팅 프로젝트 생성 (1-1)](#1-채팅-프로젝트-생성-1-1)
- [2. 서버의 종류와 데디케이티드 서버 (1-2)](#2-서버의-종류와-데디케이티드-서버-1-2)
- [3. 실습 환경 설정 (1-3)](#3-실습-환경-설정-1-3)
- [4. NetMode · NetConnection · NetDriver (1-4)](#4-netmode--netconnection--netdriver-1-4)
- [5. NetRole — Authority와 Proxy (1-5)](#5-netrole--authority와-proxy-1-5)
- [CS 35 게임플레이 프레임워크와의 연결](#cs-35-게임플레이-프레임워크와의-연결)
...
3. **챕터 1-3** — 실습 환경 설정. `IsLocalController()`로 Owning Client 판별, **Run Under One Process** 끄고 진짜 멀티 프로세스 환경 구성.
4. **챕터 1-4** — `NetMode`(월드의 네트워크 역할: StandAlone/Client/Listen/Dedicated), `NetConnection`/`NetDriver`, Ownership(소유 관계).
...
`UserWidget`을 상속한 `UCXChatInput`에 `EditableTextBox`를 `BindWidget`으로 묶는다. 핵심은 **위젯의 입력 이벤트를 델리게이트로 구독**하는 패턴:
