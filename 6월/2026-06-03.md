# 2026-06-03 언리얼 인터페이스 & 델리게이트 — HealthComponent·UI 구독·퀘스트 Subsystem + ADS 카메라 보간 + null 가드 디버깅

> 마스터반 4·5주차 언리얼 과제를 NBC_Master(UE 5.5)에서 진행. 오늘의 메인은 **Dynamic Multicast Delegate**로 체력/사망 이벤트를 발신하고 UI가 구독하는 구조, 그리고 파티 공유 퀘스트를 **GameInstanceSubsystem**에 단일 진실 공급원(SSOT)으로 두는 설계. 곁들여 우클릭 ADS 카메라 FOV/SpringArm 보간과 데미지 타입 null 포인터 크래시를 잡았다.

## 목차

- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. 4주차 — 디자인 패턴 확인 + 우클릭 ADS 조준](#1-4주차--디자인-패턴-확인--우클릭-ads-조준)
- [2. 5주차 — 인터페이스 & 델리게이트 (오늘의 메인)](#2-5주차--인터페이스--델리게이트-오늘의-메인)
- [3. 디버깅 — 데미지 타입 null 포인터 크래시](#3-디버깅--데미지-타입-null-포인터-크래시)
- [4. Git / 협업 — UE5 레포 초기화](#4-git--협업--ue5-레포-초기화)
