# 2026-06-04 UStruct vs UClass (CS 34) + 알고리즘 심화: map·set·문자열·스택
...
- [오늘 한 일 요약](#오늘-한-일-요약)
- [1. CS 33 UObject 모의면접 발표 — 네트워크→언리얼 전환점](#1-cs-33-uobject-모의면접-발표--네트워크언리얼-전환점)
- [2. CS 34 — UStruct vs UClass (오늘의 메인)](#2-cs-34--ustruct-vs-uclass-오늘의-메인)
- [3. 알고리즘 심화 — map·set·문자열·스택 7문제](#3-알고리즘-심화--mapset문자열스택-7문제)
- [4. C++ 문법 메모 (오늘 짚은 것)](#4-c-문법-메모-오늘-짚은-것)
...
2. **CS 34 "UStruct vs UClass" 준비 파일 작성** — `raw/cs-notion/34_ustruct_vs_uclass.md`. UObject 상속 여부에서 갈라지는 GC 대상·값/참조 타입·리플렉션 범위·접두사(F vs U/A)·선택 기준을 정리.
...
직전(06-02)에 네트워크 트랙의 마지막인 방화벽(CS 32)을 발표하고, 오늘 **CS 33 UObject**를 발표하며 CS 주제가 네트워크 → 언리얼 오브젝트 시스템으로 전환됐다. 발표의 뼈대는 다음과 같았다.
...
이 발표가 오늘 메인인 CS 34의 출발점이 됐다 — "UObject란 무엇인가"를 정리했으니, 다음은 "그래서 UObject로 만들 것(UCLASS)과 만들지 않을 것(USTRUCT)을 어떻게 가르는가"다.

## 2. CS 34 — UStruct vs UClass (오늘의 메인)

CS 33(UObject) 위에서 데이터를 담는 그릇을 둘로 나누는 기준을 정리했다. 준비 파일은 `raw/cs-notion/34_ustruct_vs_uclass.md`.
...
### 02번 class vs struct와의 차이

표준 C++의 class/struct는 **기본 접근 지정자(private vs public)·기본 상속 방식** 두 가지만 다르고 기능은 동일했다. 반면 언리얼의 UCLASS/USTRUCT는 **GC 대상 여부·타입 의미론(참조 vs 값)·리플렉션 범위(UFUNCTION 가능 여부)** 까지 본질적으로 다른 도구다. 02번의 구분이 "관용·스타일"의 문제였다면, 언리얼에서는 "엔진이 이 데이터를 어떻게 취급할지"를 정하는 **구조적 결정**으로 무게가 커졌다.
