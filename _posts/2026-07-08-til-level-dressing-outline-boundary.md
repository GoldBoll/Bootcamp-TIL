...
**문제**: 맵 가장자리에서 바닥이 뚝 끊기고 허공이 보인다. 인비저블월(BlockingVolume)이 있어 물리는 막혀 있지만, 시각적으로는 미완성 상태. 작업 결과는 상단 썸네일 — 가장자리에 바리케이드 줄이 서고, 그 너머로 원경 바닥과 스카이라인 임포스터가 깔려 허공이 사라졌다.
...
> **오늘 배운 것** — 커스텀뎁스 패스는 메인 패스의 가림과 무관하게 그려지기 때문에, 스텐실만 보고 아웃라인을 억제하면 배경 앞에 선 캐릭터의 윤곽까지 같이 꺼진다. "커스텀뎁스 ≈ 씬뎁스" 일치 검사로 실제로 화면에 보이는 배경 픽셀만 걸러야 한다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "포스트 프로세스 아웃라인에서 특정 영역만 선을 끄려면 어떻게 하시겠어요?" → CustomDepthStencil 규약, 커스텀뎁스-씬뎁스 일치 검사, 오클루전 하이라이트 기법 응용, PPV의 카메라 기준 한계, 픽셀 단위 판정
{: .prompt-info }
