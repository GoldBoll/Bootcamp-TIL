---
title: "2인 가구 들기 운반 모델 재설계"
subtitle: "카메라가 아니라 몸이 회전을 정하게"
date: 2026-07-13 21:30:00 +0900
categories: ["언리얼", "쿠펭"]
tags: ["ue5", "cpp", "multiplayer", "network", "netrole", "character-movement", "material", "debugging", "트러블슈팅"]
...
가구를 들고 걸으면 캐릭터가 순간이동하듯 튀는 현상이 가장 오래 남아 있었다. 값을 아무리 조정해도 안 잡혔는데, 원인은 튜닝이 아니라 구조였다 — **내 화면이 먼저 계산해 둔 이동 결과 위에 서버가 한 번 더 손을 대고 있었다**(캐릭터 무브먼트의 클라이언트 예측). 이 글에서는 그 원인을 특정한 과정과, 그 위에서 2인 공동 운반을 다시 설계한 이야기를 하려 한다 — 회전 기준을 카메라에서 몸으로 옮긴 **들것 모델**이 구현이고, 잡기 스캔이 엉뚱한 것을 집던 기하 오판 세 건이 트러블슈팅이다.

## 요구사항

- 두 사람이 가구 하나를 함께 들고 이동해도 각자 화면에서 위치가 어긋나지 않아야 한다
- 회전 기준이 카메라라서 생기는 어색한 방향 전환을 없애야 한다
- 잡기 판정이 의도한 가구를 정확히 집어야 한다

## 과정

가구가 어느 쪽으로 돌지를 정하는 기준을 카메라에서 몸으로 옮긴 다음, 그렇게 바꾸고 나서 남은 흔들림을 하나씩 잡았다.

### 기술 구현 — 협동 운반 '들것' 모델, 카메라에서 몸으로

**기존 방식의 문제**: 두 사람이 함께 든 가구가 각자 **시야가 돌아간 양**(카메라의 좌우 회전, yaw)의 가중 평균을 따라 돌았다. 몸이 아니라 카메라가 기준이었다. 가구를 든 채 시야만 돌려도 가구가 따라 돌고, 반대로 가구를 돌리지 않으려면 시야가 부자유해진다.
...
회전 기준이 카메라에서 **두 운반자를 잇는 선**으로 옮겨져, 서 있는 동료가 축이 되고 걷는 사람이 조향하는 조작 감각이 나왔다. 러버밴딩의 원인도 튜닝이 아니라 구조(예측 위에 얹힌 서버 개입)로 특정됐다.

## 트러블슈팅 1 — 운반 중 캐릭터가 순간이동한다

**증상**: 클라이언트가 가구를 들고 이동하면 캐릭터가 툭툭 뒤로 당겨지고(러버밴딩), 심하면 순간이동한다. 호스트는 멀쩡하고 **클라이언트에서만, 그것도 운반 중에만**.

**구조 분석 — 예측 파이프라인의 계약**: 이 프로젝트는 이동 로직을 직접 고쳐 쓰지 않고 엔진이 주는 무브먼트 컴포넌트(`CharacterMovementComponent`, 이하 CMC)를 그대로 쓴다. 즉 엔진 기본 클라이언트 예측·서버 보정이 그대로 살아 있고, 그 계약은 이렇다.
...
눈앞의 가구를 못 잡거나 엉뚱한 것을 잡는 일이 세 번 있었다. 원인은 전부 후보를 고르는 기하 계산이었다.
