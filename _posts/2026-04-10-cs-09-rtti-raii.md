...
description: "답변 흐름 — RAII → 스마트 포인터 → virtual 소멸자 → vtable 꼬리질문 연결 다리"
---
...
- [vtable 기본](/posts/cs-05-vtable/)
- [virtual 소멸자](/posts/cs-06-virtual-destructor/)
- [vtable deep dive](/posts/cs-08-vtable-deepdive/) — 오늘 모의면접 미답변 복기

> **오늘 배운 것** — RTTI와 RAII는 결국 vtable에서 만난다. `typeid`/`dynamic_cast`는 vtable 헤더의 `type_info*`를 읽고, RAII를 다형 클래스에 쓰려면 virtual 소멸자(= vtable)가 필수라서, 어떤 꼬리질문이 와도 vtable로 답변을 이어갈 수 있다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "dynamic_cast와 static_cast의 차이는 무엇인가요?" → 런타임 검사 유무, RTTI(vtable의 type_info), 실패 시 nullptr/bad_cast, static_cast는 UB 위험, 성능 트레이드오프(-fno-rtti)
{: .prompt-info }
