...
> **오늘 배운 것** — 그리디는 정렬 기준(종료 시간 오름차순), DP는 초기값(`dp[0]=1`), 백트래킹은 분기 검사 순서(성공→가지치기→실패)가 정답을 가른다. 가지치기는 답을 바꾸지 않고 탐색량만 줄인다는 걸 노드 수(349 vs 1701)로 직접 확인했다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "백트래킹에서 가지치기를 하면 정답이 달라질 수 있나요?" → 정답 불변, 탐색량 감소, 양수 전제라 초과 시 복귀 불가, 성공 조건 우선 검사
{: .prompt-info }
