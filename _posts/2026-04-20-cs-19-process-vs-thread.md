...
> **핵심 요약** — 프로세스와 스레드의 차이는 "자원 소유 단위 vs 실행 흐름 단위" 한 축으로 정리된다. 스레드 전환이 5~10배 빠른 이유는 TLB flush·페이지 테이블 교체·캐시 콜드를 건너뛰기 때문이고, 그 공유의 대가가 race condition과 동기화 비용이라는 것까지가 한 세트다.
{: .prompt-tip }
