...
> **핵심 요약** — 같은 격자 BFS라도 다중 시작(보물섬)은 시작점마다 dist를 리셋해야 하고 flood fill(그림)은 컴포넌트가 안 겹쳐 리셋이 필요 없다 — 기준은 "같은 칸을 다시 계산해야 하는가"다. 이벤트 시뮬레이션(NBA 농구)은 "적립 먼저, 스코어 갱신 나중" 순서와 종료 시각까지의 잔여 구간 정산이 전부였다.
{: .prompt-tip }
