...
description: "핵심 접근 — DP (0/1 냅색 / subset sum)"
...
> **오늘 배운 것** — "두 집합으로 같게 나눈다"는 "전체 합의 절반을 만드는 부분집합이 있는가"로 환원된다. 홀수 합은 즉시 false로 거르고, 1차원 DP를 역순으로 순회해야 각 원소를 한 번만 쓰는 0/1 제약이 지켜진다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "0/1 냅색 DP에서 1차원 배열을 쓸 때 왜 역순으로 순회하나요?" → 원소 중복 사용 방지, 이전 단계 값 참조 보장, 공간 O(target), subset sum 환원
{: .prompt-info }
