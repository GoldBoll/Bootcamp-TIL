...
> 21번 Context Switching 모의면접 직후 나온 후속 질문 5개를 1:1로 정리한 노트.
...
> **오늘 배운 것** — 모의면접 꼬리질문 다섯 개가 결국 한 줄로 이어진다는 것. 인터럽트가 스케줄러를 깨우고, PCB 교체 여부가 프로세스/스레드 전환을 가르고, CR3 변경이 TLB flush와 캐시 콜드를 부르고, 그 미스 누적이 프로세스 전환을 느리게 만든다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "스레드 전환이 프로세스 전환보다 빠른 이유는 뭔가요?" → 같은 가상 주소 공간, CR3 유지, TLB 보존, 캐시 히트율 유지, PCID/ASID
{: .prompt-info }
