...
> **오늘 배운 것** — skip 알파벳을 `bool[26]` 룩업 테이블에 미리 표시해 O(1)로 건너뛰고, `c - 'a'` 변환과 `% 26` 순환으로 알파벳 이동을 배열 인덱스 연산으로 바꾸는 패턴.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "특정 문자를 제외하면서 알파벳을 순환 이동시키려면 어떻게 하나요?" → bool[26] 룩업 테이블, c - 'a' 인덱스 변환, % 26 순환, skip이 아닐 때만 카운트
{: .prompt-info }
