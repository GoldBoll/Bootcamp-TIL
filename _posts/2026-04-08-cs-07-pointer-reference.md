...
![포인터는 대상의 주소를 담는 독립 변수, 레퍼런스는 같은 객체의 별칭](/assets/img/til/2026-04-08/pointer-reference-diagram.svg)
_포인터 `p`는 자기 저장소에 `x`의 주소를 담아 가리키고(재할당·nullptr 가능), 레퍼런스 `r`은 `x`와 같은 객체의 또 다른 이름이라 별도 저장소가 없다._
...
> **오늘 배운 것** — 포인터는 주소를 담는 독립 변수라 재할당·nullptr·산술 연산이 가능하고, 레퍼런스는 원본의 별칭이라 선언 시 1회 바인딩으로 끝난다. 이 차이가 함수 인자 선택 기준(`T&`, `const T&`, `T*`)과 언리얼의 `auto&` 순회 습관까지 그대로 이어진다.
{: .prompt-tip }
