...
description: "핵심 접근 — 모든 '#'을 감싸는 최소 경계 사각형(bounding box)"
...
> **오늘 배운 것** — 드래그 거리를 최소화하는 영역은 모든 `#`의 min/max 행·열로 만든 최소 경계 사각형(bounding box)으로 바로 결정되며, 좌표가 칸이 아니라 격자점이라 끝점에 +1 보정이 필요하다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "격자에서 특정 칸들을 전부 포함하는 최소 사각형은 어떻게 구하나요?" → 최소 경계 사각형, min/max 추적, 한 번 순회 O(N·M), 격자점 +1 보정
{: .prompt-info }
