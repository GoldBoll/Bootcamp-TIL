...
> **오늘 배운 것** — 세 찍기 패턴의 길이(5·8·10)가 달라도 `i % 패턴길이`로 순환 비교하면 패턴을 늘려 만들 필요 없이 한 번의 순회로 점수를 집계할 수 있다. 동점 처리는 최고점을 먼저 구한 뒤 번호 순으로 수집하면 자연히 오름차순이 된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "반복되는 패턴과 긴 배열을 비교할 때 어떻게 처리하나요?" → 모듈러 연산(%), 주기 순환, O(n) 단일 순회, 동점 오름차순 수집
{: .prompt-info }
