...
- [vector vs list](/posts/cs-13-vector-vs-list/) — 시퀀스 컨테이너의 메모리 레이아웃·CPU 캐시 분석. map 의 노드 안정성은 list 와 동일한 패턴, unordered_map rehash 무효화는 vector 재할당과 동일한 패턴
- [스마트 포인터](/posts/cs-11-smart-pointer/) — `map<Key, unique_ptr<T>>` + `try_emplace` (C++17) 패턴. 무브-온리 타입을 연관 컨테이너에 안전하게 담기
- [복사 방지(Rule of Five)](/posts/cs-12-prevent-copy/) — Rule of Five, move-only 타입. map 은 노드 안정성으로 vector 만큼 noexcept move 의존도가 낮음
- [포인터 deep dive](/posts/cs-10-pointer-deepdive/) — unordered_map rehash 후 댕글링 포인터 (vector 재할당과 동일 패턴)
- [RTTI & RAII](/posts/cs-09-rtti-raii/) — map + 스마트 포인터 합성 RAII
- 00_index.md(미발행 내부 노트) — CS 면접 인덱스
