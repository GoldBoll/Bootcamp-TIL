...
description: "답변 흐름 — 모의면접 후속 주제: '`std::map`은 어떻게 동작하나요? `unordered_map`과 어떻게 다른가요?'"
---
...
map 은 insert·erase·find·operator[] 전부 O(log n), unordered_map 은 전부 평균 O(1)·최악 O(n) 입니다. 순회는 둘 다 O(n) 이지만 map 만 정렬 순서가 보장되고, 범위 조회(lower/upper_bound) 는 map 만 O(log n) 으로 지원합니다. 연산별 전체 표는 [4번](#4-주요-연산과-시간-복잡도)에, 두 컨테이너 정량 비교표는 [6번](#6-유사한-범위의-다른-컨테이너--set--multimap--unordered_) 끝에 있습니다.

### Red-Black Tree 5속성 30초

다섯 개 중 답변의 뼈대는 두 개입니다 — "빨강 연속 금지(속성 4)" 와 "모든 경로의 검정 노드 수 동일(속성 5)". 이 둘이 트리 높이를 최악 2 log(n+1) 로 묶어 O(log n) 을 보장합니다. 속성 다섯 개 전체와 높이 제한이 나오는 이유는 [3번](#3-내부-동작--red-black-tree)에 한 번만 정리했습니다.

### 꼬리질문 연결 맵

메인 질문에서 가지가 뻗는 방향은 크게 일곱 갈래입니다 — Red-Black Tree 내부(가장 깊이 들어감), 시간 복잡도, unordered_map 비교(가장 자주 나옴), iterator 무효화, operator[] 함정, 메모리 오버헤드, 언리얼 TMap. 전체 트리는 [11번](#11-꼬리질문-예상-경로)에 한 번만 그렸습니다.
...
C++11 이후 hint 가 정확(직전 위치) 하면 amortized O(1) — 여러 번의 삽입을 평균 내면 O(1) 이라는 뜻 — 으로 처리됩니다. 정렬된 입력을 통째로 넣을 때 효과가 큽니다.
...
> map 가족은 **(정렬 vs 해시) × (키만 vs 키-값) × (중복 허용 vs 불허)** 의 8가지 조합입니다. 가장 자주 쓰는 4가지를 먼저 익히고 나머지는 표로 기억하면 됩니다.
...
| 범위 조회 (lower/upper_bound) | `map` / `set` — 해시는 불가 |
...
따라서 새 노드는 **항상 빨강** — 복구 비용을 최소화하는 선택입니다.
...
내부 구조:
- 해시 테이블 + open addressing(충돌 시 연결 리스트를 만들지 않고 배열의 다른 슬롯을 찾아가는 방식) 변형 — 캐시 친화를 위해 체이닝을 쓰지 않음
...
rehash 가 일어나면 기존 포인터는 전부 댕글링(이미 해제된 메모리를 가리키는 포인터) 이 됩니다. 13번 vector 재할당과 동일한 패턴 — 연속 메모리/버킷 배열 자료구조의 공통 함정입니다.
...
8번에서 코드로 뜯어본 내용을, 실제 면접에서 입으로 나올 30초 분량으로 압축한 버전입니다.
...
**Q: map 과 unordered_map 언제 어떤 걸 쓰나요?** (가장 자주 나옵니다)
```
map 선택:
  - 키 정렬이 결과적으로 필요
...
> **오늘 배운 것** — std::map 은 Red-Black Tree 기반이라 insert·erase·find 가 전부 O(log n) 최악 보장이고, 삽입 시 iterator 무효화가 없다. unordered_map 의 평균 O(1) 과는 트레이드오프가 정반대라, 정렬·범위 조회·최악 시간 보장이 필요할 때만 map 을 고르면 된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "std::map 과 unordered_map 은 각각 언제 쓰나요?" → Red-Black Tree, O(log n) 최악 보장, 해시 평균 O(1)·최악 O(n), 범위 조회(lower_bound), rehash iterator 무효화
{: .prompt-info }
