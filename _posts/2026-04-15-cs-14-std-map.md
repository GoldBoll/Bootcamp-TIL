...
image: /assets/img/thumbs/cards/2026-04-15-cs-14-std-map.svg
description: "답변 흐름 — 모의면접 후속 주제: '`std::map`은 어떻게 동작하나요? `unordered_map`과 어떻게 다른가요?'"
---

# std::map (Red-Black Tree 기반 정렬 연관 컨테이너)
...
> **핵심 요약** — std::map 은 Red-Black Tree 기반이라 insert·erase·find 가 전부 O(log n) 최악 보장이고, 삽입 시 iterator 무효화가 없다. unordered_map 의 평균 O(1) 과는 트레이드오프가 정반대라, 정렬·범위 조회·최악 시간 보장이 필요할 때만 map 을 고르면 된다.
{: .prompt-tip }
