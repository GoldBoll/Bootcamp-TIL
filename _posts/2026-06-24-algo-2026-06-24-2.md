...
> **오늘 배운 것** — 3Sum은 정렬이 투 포인터의 전제라는 걸 보여주는 문제다. 첫 수를 고정하고 나머지를 양끝에서 좁히면 O(n³)이 O(n²)로 떨어지고, 중복 제거도 set 없이 정렬 순서(직전 값과 같으면 스킵)만으로 해결된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "3Sum을 O(n²)로 풀고 중복 트리플은 어떻게 제거하는가?" → 정렬 선행, 첫 수 고정 + 투 포인터, 합의 부호로 이동 방향 결정, 직전 값 스킵으로 중복 제거, nums[i] > 0 가지치기
{: .prompt-info }
