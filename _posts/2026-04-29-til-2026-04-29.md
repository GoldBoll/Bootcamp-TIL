---
title: "[TIL] 2026-04-29 — 8번 과제 종결(D5) — 레벨 전환·HUD·디버프"
date: 2026-04-29 09:00:00 +0900
categories: ["언리얼"]
tags: ["daily", "til"]
render_with_liquid: false
description: "8번 과제 종결 D5 — 레벨 전환 시스템, HUD 4페이지 모드, 좀비 처치 점수·인벤토리 스택, 디버프 3종과 게임오버 처리까지 마무리했다."
...
- [2026-04-29 8번 과제 종결 (Day 5) — 차량 슬롯 마무리 + 레벨 전환 + HUD 4페이지 + 디버프 발동](#2026-04-29-8번-과제-종결-day-5--차량-슬롯-마무리--레벨-전환--hud-4페이지--디버프-발동)
  ...
  - [HUD 4페이지 모드 — 단일 위젯 가시성 토글](#hud-4페이지-모드--단일-위젯-가시성-토글)
    ...
    - [버튼 자동 바인딩 — `AddUniqueDynamic`](#버튼-자동-바인딩--adduniquedynamic)
...
마감 D-2였는데 오늘 8번 과제 본체 기능을 전부 닫았다. 오전 PIE(에디터에서 바로 실행하는 플레이 테스트) 검증에서 차량 부품 슬롯이 블로커 6개 — 헬기 콜리전, ChildActor Mobility, `AddIgnoredActor` 루프, Sweep 차단, `OwnerVehicle` 설정 불가, `Slot` 변수명 가림 — 에 연쇄로 막혔고, 이걸 전부 풀어 시동 분기 진입까지 통과시켰다. 이어서 레벨 3개(`Lv_Main` 메뉴 / `Lv_VoidProto` 본편 / `Lv_Escape` 탈출 엔딩) 전환 흐름과 10분 탈출 타이머를 붙이고, `GameStateClass`를 C++ 생성자에서 강제해 BP 캐싱 회귀를 차단했다.

HUD는 단일 `WBP_HUD` 안 두 컨테이너로 MainMenu / InGame / GameOver / GameClear 4페이지를 통합했다. 레벨명 기반 자동 모드 결정, `AddUniqueDynamic` 버튼 자동 바인딩, 레벨별 Exit 분기까지 붙였고, 바인딩 이전에 지나간 Broadcast는 현재값으로 핸들러를 1회 직접 호출해 보충하는 패턴으로 잡았다. 게임오버는 레벨 전환 없이 HUD 모드만 토글해 시체·플레이 상태를 배경에 남겼다.

그 외에 인벤토리 스택(같은 ItemData 누적, `MaxStackPerItem=2`), 좀비 처치 점수, `UVOIDWeaponConfig` 메타 필드(DisplayName/Icon), 시작 무기 샷건 + 좀비 데미지 20→10 조정, 디버프 트리거 2종(무게 비율 Overweight / 피격 확률 Bleeding·Fracture)과 Bleeding 도트, `HealthComponent` 누락 시 런타임 fallback까지 마무리. 커밋은 5개 파트로 나눠 올렸고, 저녁엔 vector vs list 모의면접을 준비했다.
...
| # | 메시지 | 범위 |
|---|---|---|---|
| 1 | Finish vehicle part slot system — engine start branch + auto-find OwnerVehicle | 차량 슬롯 마무리 |
| 2 | GameMode flow — escape/gameover level transitions + 10min escape timer | 레벨 전환 + 타이머 + 게임오버 |
| 3 | HUD system — 4-mode panel (Menu/InGame/GameOver/GameClear) + auto level routing | HUD 4페이지 |
| 4 | Inventory stacking + score system + zombie balance pass | 인벤토리 스택 + 점수 |
| 5 | Debuff system trigger + health logging + interact sweep fallback + default shotgun | 디버프 + Health + 시작 무기 |
...
> **오늘 배운 것** — 어제 임시로 넣은 `AddIgnoredActor` 루프가 오늘 시동 분기를 영구 차단하는 회귀로 돌아왔다. 검증할 때는 어제 추가분부터 다시 의심해야 한다. 그리고 델리게이트는 바인딩 이전 Broadcast가 사라지므로, 바인딩 직후 현재값으로 핸들러를 1회 직접 호출하는 패턴을 표준으로 삼았다.
{: .prompt-tip }
