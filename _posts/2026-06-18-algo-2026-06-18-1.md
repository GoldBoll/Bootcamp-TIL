...
> **오늘 배운 것** — 알고리즘보다 "여러 번 신고해도 1회" 조건이 함정인 문제였다. "신고자 피신고자" 문자열을 통째로 set에 넣어 맨 앞에서 중복을 거르고, 정지 판정용·메일 배달용 맵을 나누면 report 한 번 훑기로 끝난다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "중복을 걸러내면서 집계해야 할 때 어떤 자료구조를 쓰나요?" → unordered_set, insert 반환값 bool, unordered_map 카운팅, O(report) 선형 처리
{: .prompt-info }
