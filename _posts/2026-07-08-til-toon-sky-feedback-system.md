...
**재사용**: 아무 레벨에나 붙일 수 있게 적용 절차를 문서화했다 — ① 돔 머티리얼 교체 ② 볼류메트릭 구름 숨김 ③ 스카이라이트 실시간 캡처 ④ 동기화 액터 배치. 적용된 도시 전경은 상단 썸네일 — 라이트 각도에 따라 낮→황혼→밤이 실시간으로 전환된다.
...
> **오늘 배운 것** — Unlit 이미시브 하늘은 라이팅 파이프라인과 무관해 라이트를 알 수 없다. MPC에 태양 방향을 기록하는 동기화 액터로 다리를 놓았고, 팀 파일을 한 줄도 수정하지 않는 확장은 서브시스템(자동 부착)+컴포넌트(관찰)+인터페이스(옵트인) 구조로 풀 수 있었다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "기존 코드를 수정하지 않고 기능을 확장하려면 어떻게 설계하시겠어요?" → 월드 서브시스템 자동 부착, 컴포넌트 관찰, 인터페이스 옵트인, 리플렉션 프로퍼티 읽기, 복제값 관찰
{: .prompt-info }
