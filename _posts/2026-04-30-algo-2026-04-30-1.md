...
> **오늘 배운 것** — 단순 재귀는 같은 값을 반복 계산해 O(2^n)으로 폭발한다(n=30이면 약 10억 호출). 이전 두 값만 변수로 유지하며 앞에서부터 계산하는 Bottom-up 반복이면 시간 O(n), 공간 O(1)로 끝난다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "재귀 피보나치가 느린 이유와 개선 방법은?" → 중복 계산, 메모이제이션(Top-down), 반복 DP(Bottom-up), O(2^n)→O(n), 공간 O(1)
{: .prompt-info }
