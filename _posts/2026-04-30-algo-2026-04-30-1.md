...
image: /assets/img/thumbs/cards/2026-04-30-algo-2026-04-30-1.svg
---
...
> **핵심 요약** — 단순 재귀는 같은 값을 반복 계산해 O(2^n)으로 폭발한다(n=30이면 약 10억 호출). 이전 두 값만 변수로 유지하며 앞에서부터 계산하는 Bottom-up 반복이면 시간 O(n), 공간 O(1)로 끝난다.
{: .prompt-tip }
