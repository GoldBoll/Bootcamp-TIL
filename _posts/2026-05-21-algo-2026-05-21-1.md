...
> **오늘 배운 것** — 내림차순으로 정렬하면 매 m번째 원소가 곧 그 상자의 최저점이 된다. 최저점에 m을 곱해 더하기만 하면 되고, m개를 못 채우는 남는 사과는 인덱스 조건에서 자동으로 제외된다 — O(n log n).
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "상자 가격이 최저점×m으로 정해질 때 왜 내림차순 정렬로 최대 이익이 보장되나요?" → 내림차순 정렬, 매 m번째 원소 = 상자 최저점, 남는 사과 자동 제외, O(n log n)
{: .prompt-info }
