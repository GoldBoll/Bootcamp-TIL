...
> **오늘 배운 것** — 격자 이동 시뮬레이션은 목적지만 검사하면 틀린다. 한 칸씩 전진하며 중간 칸까지 전부 검사하고, 임시 좌표로 진행한 뒤 전 구간을 통과했을 때만 실제 좌표에 반영해야 실패 시 위치가 오염되지 않는다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "격자 시뮬레이션에서 이동 명령을 검증할 때 주의할 점은?" → 중간 경로 검사, 임시 좌표 시뮬레이션, 경계 검사, dr/dc 방향 증분, 원자적 반영
{: .prompt-info }
