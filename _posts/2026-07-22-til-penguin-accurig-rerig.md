...
리깅 재작업의 목적지는 "펭귄이 기성 애니메이션을 받아 움직이는 것"이다. 이 부분은 팀 프로젝트(TeamCarry) UE 에디터를 원격 조회(unreal-mcp, 읽기 전용)해 실제 연결 구조를 확인했다 — 애니메이션 임포트는 팀 공동 작업(임포트 경로 기준 팀원과 내 다운로드 폴더 양쪽)이라, "무엇이 어떻게 붙어 있는가"를 사실 위주로 정리한다.
