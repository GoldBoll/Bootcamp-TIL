...
image: /assets/img/thumbs/cards/2026-06-30-algo-2026-06-30-2.svg
---
...
> **핵심 요약** — 가변 크기 슬라이딩 윈도우에서 left를 한 칸씩 줄이지 않고 `last[c] + 1`로 한 번에 점프시키면 두 포인터가 단조 증가만 해서 O(n)이 된다. 이때 `last[c] >= left` 비교로 "현재 윈도우 안의 중복"인지 판단하는 게 관건이다.
{: .prompt-tip }
