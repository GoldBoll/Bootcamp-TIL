---
title: "LeetCode 35 - Search Insert Position (Easy)"
date: 2026-05-12 13:00:00 +0900
categories: ["알고리즘", "LeetCode"]
tags: ["algorithm"]
render_with_liquid: false
description: "정렬된 distinct integer 배열 nums와 target이 주어진다."
...
> **오늘 배운 것** — "target이 삽입될 위치"는 곧 "target 이상인 첫 원소의 인덱스", 즉 lower_bound 그대로다. [left, right) 반열린 구간으로 짜면 값이 존재하는 경우와 없는 경우를 분기 없이 한 코드로 처리할 수 있다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "정렬된 배열에서 값의 삽입 위치를 O(log n)에 찾는 방법을 설명해 보세요." → 이분 탐색, lower_bound, 반열린 구간 [left, right), mid 오버플로 회피(left + (right - left) / 2)
{: .prompt-info }
