...
- 왜 그 비교자가 옳은가는 **국소 교환 논증**으로 본다. 최적 배치에서 인접한 두 수 a, b를 뒤집었을 때 더 커지는 경우가 없어야 하고, 인접 교환은 앞뒤 문맥과 무관하게 `a+b` vs `b+a`만 바꾼다. 즉 "인접쌍이 모두 최적"이 곧 전체 최적이고, 그런 순서를 만들어 주는 것이 정렬이다. 정렬로 푸는 그리디의 정당성은 대부분 이 형태로 검증된다(백준 1931 회의실 배정에서 끝나는 시각을 기준으로 삼는 것과 같은 계열).
