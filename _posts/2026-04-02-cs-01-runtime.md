...
description: "컴파일 타임과 런타임의 차이, RTTI와 런타임 에러를 면접 답변 흐름으로 정리"
---
...
> **오늘 배운 것** — 런타임은 컴파일된 프로그램이 OS 위에서 실제로 실행되는 단계이고, 동적 메모리 할당·동적 디스패치·RTTI가 모두 이때 일어난다. `constexpr`·템플릿·`static_assert`로 비용을 컴파일 타임으로 옮길 수 있으며, 언리얼의 `Cast<T>()`와 GC도 결국 런타임 메커니즘이라는 점을 연결해서 정리했다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "컴파일 타임과 런타임의 차이는 무엇인가요?" → 빌드 시 vs 실행 시, 타입 검사·최적화, 동적 디스패치·RTTI, 런타임 에러·UB, constexpr로 비용 이동
{: .prompt-info }
