...
image: /assets/img/thumbs/cards/2026-04-02-cs-01-runtime.svg
...
> **핵심 요약** — 런타임은 컴파일된 프로그램이 OS 위에서 실제로 실행되는 단계이고, 동적 메모리 할당·동적 디스패치·RTTI가 모두 이때 일어난다. `constexpr`·템플릿·`static_assert`로 비용을 컴파일 타임으로 옮길 수 있으며, 언리얼의 `Cast<T>()`와 GC도 결국 런타임 메커니즘이라는 점을 연결해서 정리했다.
{: .prompt-tip }
