...
> **오늘 배운 것** — "같은 사람을 여러 번 신고해도 1회" 조건 때문에 중복 제거가 모든 집계의 전제다. 그리고 정지 판정(피신고자→횟수)과 메일 배달(피신고자→신고자들)은 조회 방향이 달라서 맵을 둘로 나누는 게 깔끔하다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "해시 기반 자료구조로 중복 제거와 카운팅을 어떻게 처리하나요?" → unordered_set 중복 제거, insert().second 활용, unordered_map 카운팅, 평균 O(1), 조회 방향별 맵 분리
{: .prompt-info }
