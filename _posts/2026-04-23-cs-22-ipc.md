...
description: "답변 흐름 — 왜 IPC가 필요한가(주소 공간 격리·커널 모드 경유) → 분류축 3개(데이터 vs 신호 / 커널 경유 vs 사용자 경유 / 같은 머신 v"
---

# 📕 05/13 — 프로세스끼리 통신할 수 있는 방법(IPC, Inter-Process Communication)에 대해 설명해 주세요
...
17. [핵심 요약 카드 (재게재)](#17-핵심-요약-카드-재게재)
...
데이터 전송 채널은 **무엇을 보냈는가**가 핵심이고, 신호 전송은 **언제 무슨 일이 일어났는가**가 핵심입니다. 실무에선 둘을 짝지어 씁니다 — 공유 메모리에 데이터를 쓰고 세마포어로 "다 썼다"고 알리는 식. **메일슬롯에 작은 명령을 넣고 받는 쪽이 Event 객체로 깨어나는 식.**
...
| **Low** | 인터넷 Explorer protected mode, Chrome 렌더러 |
| **Untrusted** | 더 강한 샌드박스 |
...
```
IPC = OS의 주소 공간 격리를 깨지 않으면서 프로세스 간 데이터·신호를 교환하는 메커니즘.
      각 프로세스가 자기 페이지 테이블 → 가상 주소가 달라 → 반드시 커널이 중재.
...
메커니즘 (빠름 → 느림):
  공유 메모리             — CreateFileMapping/MapViewOfFile, shm_open+mmap
                            가장 빠름(수 ns), 동기화 직접 처리 (Mutex+Sem+Event와 짝)
  메모리 맵 파일          — 디스크 파일을 메모리로 (page cache 공유)
  파이프                  — 단방향 바이트 스트림
                            익명: 부모-자식만 / 명명: 무관·네트워크 가능 (SMB)
  메시지 큐               — 메시지 경계 보존
                            POSIX mq (우선순위), Windows PostMessage·Mailslot
  Mutex/Sem/Event (Named) — 동기화 신호, 프로세스 간 공유 (Wait 시 커널)
  시그널 (POSIX)          — 비동기 알림, async-signal-safe 제약
                            Windows: Event + WaitForSingleObject로 대응
  소켓                    — 가장 범용
                            UNIX domain (로컬, 빠름) / TCP/UDP (네트워크)
  RPC / COM               — 함수 호출처럼, marshalling 비용 큼

비용 스펙트럼 (낮음 → 높음):
  공유 메모리 접근            수 ns
  Mutex/Event signal+wait     1~3 μs
  Anonymous Pipe              수 μs
  Named Pipe (loopback)       수 μs
  UNIX domain socket          수 μs
  TCP loopback                5~30 μs
  PostMessage                 수 μs
  Mailslot                    수십 μs
  MS-RPC (LRPC)               수십 μs
  MS-RPC (TCP)                수십~수백 μs
