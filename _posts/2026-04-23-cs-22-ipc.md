...
> **오늘 배운 것** — 모든 IPC는 주소 공간 격리를 커널 중재로 우회하는 방법이고, 공유 메모리(수 ns)부터 RPC/COM(수십~수백 μs)까지 같은 "데이터 옮기기"에 1000배 이상 비용 차이가 난다. 그래서 선택 기준은 빈도·데이터 크기·신뢰 경계이고, 실무 기본 패턴은 공유 메모리(데이터) + Named 동기화 객체(신호)의 분업이다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "프로세스 간 통신 방법에는 어떤 것들이 있고, 어떤 기준으로 고르나요?" → 주소 공간 격리, 커널 중재, 파이프/메시지 큐/공유 메모리/소켓/RPC, 비용 스펙트럼, 공유 메모리+세마포어 조합
{: .prompt-info }
