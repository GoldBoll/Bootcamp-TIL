---
title: "[TIL] 2026-04-24 — Void TPS 피벗 · 8번 과제 레포 구축 · 팀플 기여 가이드"
date: 2026-04-24 09:00:00 +0900
categories: ["언리얼"]
tags: ["daily", "til"]
render_with_liquid: false
description: "Void GDD를 TPS로 피벗해 9개 섹션 재작성, 8번 과제 레포 구축과 7일 일정 재조정, TPS PlayerCharacter 코드 전환과 팀플 기여 가이드 수립."
...
결정이 많은 날이었다. Void GDD(게임 디자인 문서)를 쿼터뷰에서 3인칭 TPS로 피벗하면서 `game-concept.md`의 9개 섹션(피치·필러·MDA·Core Loop·비주얼·레퍼런스·기술 리스크·Feature List)을 재작성했고, 3일 밀린 8번 과제 일정을 10일 → 7일로 재조정해 오늘을 실질적 Day 1로 삼았다(`8번과제_구현계획.md` §6·§9 재작성). 과제 제출용으로 `NBC_JangSik_Assignment8` Private 레포를 만들어 초기 3커밋을 push했는데, 이 과정에서 gitignore를 `Content/VoidUnreal/`만 추적하는 화이트리스트 패턴으로 재설계했다. 코드는 `AVOIDPlayerCharacter`를 TPS 숄더뷰 기준(SpringArm 1400→300, `bUseControllerRotationYaw=true`)으로 전환하고 Move/Look을 재작성했다.

팀프로젝트 쪽에서는 CONTRIBUTING.md + PR 템플릿으로 기여 규칙(GitHub Flow, Conventional Commits, 담당 영역)을 세우고, 레포 기본 브랜치를 master → main으로 전환했다. 수업에서는 `UDamageType` 상속 커스터마이징과 동기/비동기 Trace 시스템을 배웠는데(`D:\Unreal\NBC_Master` 실습), 이게 그대로 다음주 과제 — 필수: 샷건 + 커스텀 반동, 도전: 비동기 Trace AI 탐지, 마감 5/1 — 의 기반 기술이다.
...
**원래 고민**: `game-concept.md`는 쿼터뷰 기반으로 설계되어 있는데, 팀프로젝트(Ch3)가 TPS로 결정됐다. 내 역할(C — 전투/히트 판정/히트스톱/처형)에서 쌓을 TPS 타격감 자산을 Void 본편으로 이식하려면 **시점을 TPS로 통일**해야 재사용이 자연스럽다.

**검토 결과**: 장르(익스트랙션 — 파밍 후 탈출까지가 한 판인 구조)·필러(Noise is Currency)·레퍼런스(타르코프·좀보이드)는 대부분 유지 가능. **시점 관련 문장만 교체**하면 되므로 전면 재작성이 아닌 **부분 재작성** 스코프.
...
> **오늘 배운 것** — 컨셉 피벗은 전면 재작성이 아니라 "시점에 의존하는 섹션만 골라 교체"하는 범위 관리 문제였다. 그리고 UE 프로젝트는 첫 커밋 전에 gitignore(화이트리스트 패턴)를 끝내둬야 578MB 에셋 같은 대용량 파일 때문에 커밋 이력을 다시 쓰는 일을 피할 수 있다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "동기 트레이스와 비동기 트레이스를 어떤 기준으로 구분해서 쓰나요?" → 호출 즉시 반환 vs 콜백 수신, 메인 스레드 블로킹, 다수 AI 시야 판정에 비동기, 1~2프레임 지연 감안, FTraceDelegate
{: .prompt-info }
