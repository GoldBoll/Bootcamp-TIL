...
7. **오늘 결정은 3개 레포에 걸쳐 일관성을 확보했다.** 게임 기획 GDD 레포(TPS로 재작성) → `NBC_JangSik_Assignment8` 코드(TPS 카메라 리그) → `NBC_Ch3_TeamProject` 규칙(CONTRIBUTING+main). 한 결정이 다른 레포로 파급될 때 **끊김 없이 이어지는 구조**가 나중에 자산 이식을 가능하게 만든다.
