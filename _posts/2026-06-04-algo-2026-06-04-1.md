...
> **오늘 배운 것** — (i, j)에는 위 또는 왼쪽에서만 올 수 있으므로 `grid[i][j] += min(위, 왼쪽)` 점화식으로 풀리고, 입력 격자를 dp 테이블로 덮어쓰면 추가 공간 없이 O(1)로 처리된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "격자 최소 경로합 DP의 점화식과 공간 최적화를 설명해 보세요." → 위/왼쪽 두 방향 한정, min 누적 점화식, 첫 행·첫 열 예외 처리, in-place 덮어쓰기 O(1)
{: .prompt-info }
