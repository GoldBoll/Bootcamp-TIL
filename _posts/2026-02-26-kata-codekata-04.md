...
> **오늘 배운 것** — 태어난 해에 1살로 세는 나이 체계에서는 `출생 연도 = 기준 연도 - 나이 + 1`. 빼기만 하고 +1 보정을 빼먹기 쉬운, 전형적인 하나 차이(off-by-one) 문제다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "off-by-one 오류를 어떻게 예방하나요?" → 기준점 정의 확인, 경계값 대입 검증, 제한사항 확인, 작은 예시로 손 계산
{: .prompt-info }
