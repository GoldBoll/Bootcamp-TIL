...
> **오늘 배운 것** — 문자 빈도는 종류가 한정돼 있으면 map 대신 고정 크기 배열로 세는 게 낫다. 최댓값과 동률 여부는 dup 플래그 하나로 한 번의 순회에서 같이 판정할 수 있다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "문자열에서 특정 문자의 빈도를 셀 때 어떤 자료구조를 쓰겠는가?" → 고정 크기 배열 카운팅, O(N) 순회, 대소문자 toupper 통일, map 대비 오버헤드, 동률 플래그
{: .prompt-info }
