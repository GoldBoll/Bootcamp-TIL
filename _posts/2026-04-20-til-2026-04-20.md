---
title: "UE C++ 6DOF 드론 Pawn 중력·착지 구현"
subtitle: "이동 전 예측 트레이스로 통통 튀는 착지 잡기"
date: 2026-04-20 09:00:00 +0900
categories: ["언리얼"]
tags: ["daily", "til", "트러블슈팅"]
render_with_liquid: false
description: "이동 3축·회전 3축을 전부 플레이어가 쥐는 드론이라 CharacterMovement가 없다. 중력과 착지 스냅을 직접 짜야 했고, 이동 후에 스냅하는 순서 때문에 드론이 바닥에서 튀었다."
image: /assets/img/thumbs/cards/2026-04-20-til-2026-04-20.svg
---

드론은 걷는 캐릭터와 달리 이동 3축 + 회전 3축, 여섯 자유도를 전부 플레이어가 쥔다. 그래서 `CharacterMovementComponent`가 대신 해 주던 중력과 착지를 직접 짜야 했고, 그 지점에서 문제가 몰려 나왔다. 이 글에서는 C++ Pawn으로 6DOF 드론을 만든 과정을 이야기하려 한다 — 구현은 Enhanced Input 매핑, Pawn 컴포넌트 구조, 중력과 착지 스냅 물리이고, 트러블슈팅은 **바닥에서 통통 튀는 현상**을 비롯한 네 건이다.

## 기술 구현 1 — Enhanced Input 세팅 (IMC_Drone)
...
> **핵심 요약** — 바닥 뚫기(터널링)는 이동 *후*가 아니라 이동 *전*에 예측 위치를 LineTrace로 검사해야 바운스 없이 막을 수 있다. 그리고 `GetActorUpVector()`는 월드 공간 벡터라 `AddActorLocalOffset`에 넘기면 이중 회전이 걸린다 — 액터 Up 방향 이동은 `AddActorWorldOffset`으로 써야 한다.
{: .prompt-tip }
