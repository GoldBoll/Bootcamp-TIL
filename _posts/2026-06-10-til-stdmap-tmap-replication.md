...
프로그래머스 161990 "바탕화면 정리"는 모든 파일(`#`)을 감싸는 **최소 경계 사각형(bounding box)** 을 구하는 문제다. 드래그로 한 번에 모든 파일을 선택할 때, 시작점과 끝점을 `[r1, c1, r2, c2]`로 반환한다.

핵심 직관은 **드래그 거리는 파일들의 최소/최대 행·열로 항상 결정된다**는 것이다. 어떤 파일도 빠뜨리면 안 되니 가장 위/왼쪽 파일과 가장 아래/오른쪽 파일이 경계를 정한다. 그래서 전체 격자를 한 번만 순회하며 `#` 칸의 최소/최대 행·열을 갱신하면 O(N·M) 단일 스캔으로 끝난다.
...
함정은 끝점의 +1 보정이다. 좌표가 **칸**이 아니라 **격자점**을 가리키므로, 시작점은 가장 왼쪽 위 파일 칸의 왼쪽 위 격자점(`r1, c1`)이지만, 끝점은 가장 오른쪽 아래 파일 칸의 **오른쪽 아래** 격자점이라 `r2+1, c2+1`이 된다. `r1`은 큰 값(n), `r2`는 작은 값(-1)으로 초기화해 단일 스캔에서 min/max가 자연스럽게 잡히게 했다. MSVC 컴파일 후 예제 4개 모두 통과했다.
