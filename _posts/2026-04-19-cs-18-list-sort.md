...
> **오늘 배운 것** — `std::sort`는 RandomAccessIterator를 요구하는 introsort라서 list에는 컴파일부터 안 되고, `std::list::sort`는 노드 포인터 재연결만으로 정렬하는 stable한 병합 정렬이다. "자료구조 특성을 알고리즘이 못 살릴 때 멤버 함수가 따로 존재한다"는 컨벤션이 set/map::find와 같은 패턴으로 이어진다는 것.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "std::list::sort()가 std::sort() 대신 따로 존재하는 이유는 무엇인가요?" → iterator 카테고리, RandomAccessIterator, 병합 정렬, 노드 재연결, stable sort
{: .prompt-info }
