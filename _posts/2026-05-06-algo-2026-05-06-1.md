---
title: "LeetCode 50 - Pow(x, n) (Medium)"
date: 2026-05-06 13:00:00 +0900
categories: ["알고리즘", "LeetCode"]
tags: ["algorithm"]
render_with_liquid: false
description: "pow(x, n) 구현 — x의 n제곱을 반환"
image: /assets/img/thumbs/leetcode.svg
---

> 출처: <https://leetcode.com/problems/powx-n/>

```cpp
// LeetCode 50 - Pow(x, n) (Medium)
// https://leetcode.com/problems/powx-n/

// 문제 설명
// pow(x, n) 구현 — x의 n제곱을 반환

// 제약 조건
// -100.0 < x < 100.0
// -2^31 <= n <= 2^31-1  (n이 음수일 수 있음)
// n은 정수
// x != 0 이거나 n > 0
// -10^4 <= x^n <= 10^4

// 예시
// x = 2.00000, n =  10 →  1024.00000
// x = 2.10000, n =   3 →     9.26100
// x = 2.00000, n =  -2 →     0.25000  (2^-2 = 1/4 = 0.25)

// 풀이 비교
// 단순 반복        : 시간 O(n)      — n이 2^31이면 TLE
// 분할 정복 (재귀)  : 시간 O(log n)  — x^n = x^(n/2) * x^(n/2), n 홀수면 x 한 번 더 곱함
// 빠른 거듭제곱 반복 : 시간 O(log n)  — 비트 단위 처리, 스택 없음 → 최적
// 주의: n = INT_MIN (-2^31) 일 때 -n 오버플로우 → long long으로 변환 필요

class Solution {
public:
    double myPow(double x, int n) {
        long long N = n;
        if (N < 0) {
            x = 1.0 / x;
            N = -N;
        }
        double result = 1.0;
        while (N > 0) {
            if (N & 1) result *= x;
            x *= x;
            N >>= 1;
        }
        return result;
    }
};
```

> **오늘 배운 것** — 거듭제곱을 O(n) 반복 대신 비트 단위 빠른 거듭제곱으로 O(log n)에 계산했다. 음수 지수는 역수로 바꿔 처리하되, n = INT_MIN이면 -n이 int 범위를 넘어가므로 long long으로 먼저 받아야 한다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "pow(x, n)을 O(log n)으로 구현하는 방법은?" → 분할 정복, 비트 단위 반복(N&1·N>>=1), 음수 지수 역수 처리, INT_MIN 오버플로, long long 변환
{: .prompt-info }

