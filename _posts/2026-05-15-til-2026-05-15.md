...
description: "Ch3 HitReact·히트스톱을 단일 발사 경로로 통합 — 콜리전 채널 정합, 본 폴백, 연사 RESTART 로직을 구현하고 발사 흐름과 설계 결정을 정리했다."
...
내일 팀 코드리뷰를 앞두고 Ch3 팀플의 HitReact(피격 시 본이 흔들리는 반응)와 히트스톱(타격 순간 시간을 잠깐 늦춰 타격감을 주는 연출)을 하나의 발사 경로로 통합했다. 오늘의 결론을 한 줄로 줄이면 — 좌클릭이든 BP Fire든 AnimNotify든 모든 발사가 `UWeaponComponent::TryFire` 한 곳으로 모이게 통일해서, 어느 호출 경로에서도 HitReact·히트스톱이 동일하게 발동하게 만들었다.

작업 순서대로:

1. **테스트 환경 셋업** — `BP_TestPlayerCharacter` 신규(BP_PlayerCharacter 상속). World Settings의 GameMode Override가 BP_NBC_GameMode로 박혀 있어 Project Settings의 BP_TestGameMode가 무시되던 함정을 World Settings에서 직접 교체. PlayerController BP에 IMC(Input Mapping Context) + IA_Move/Look/Jump/Fire 4개 액션을 직접 할당해야 입력이 도는 구조 파악
2. **ZombieCharacter 콜리전 정합** — `UHitReactComponent`를 C++ `CreateDefaultSubobject`로 자동 부착. Mesh는 **ECC_Weapon** 포함 Block, Capsule은 Ignore로 코드 일괄 설정 — 라인트레이스가 캡슐을 통과해 본까지 도달하도록 정리. 처음에 이 채널 응답 누락으로 트레이스가 캡슐에 막혀 BoneName=None이 떨어지던 이슈 해결
3. **WeaponComponent — BoneName 폴백** — `Hit.BoneName.IsNone()`이면 `FindClosestBone_K2`로 가장 가까운 본을 찾고, `_End` 말단 본은 부모로 최대 8단계 거슬러 올라가 처리. PhysicsAsset 본 콜리전이 듬성듬성해도 시각 반응 유지
4. **WeaponComponent — 히트스톱** — 좀비 Pawn + 그 `AIController` + 플레이어까지 `CustomTimeDilation = 0.1`로 동시 적용 후 0.15초 뒤 1.0 복귀. **AIController도 같이 멈춰야 BT(Behavior Tree, AI 의사결정 트리)가 진짜로 멎는다** — Pawn만 멈추면 BT는 그대로 굴러서 "타격감 없는 추격"이 됨
5. **HitReactComponent — 연사 RESTART** — 연사 중 다른 본이 맞으면 이전 본을 즉시 정리하고 새 본에서 재시작. `bIsReacting` 가드 때문에 연사 두 번째 발 이후가 통째로 무시되던 버그 제거
6. **BaseWeapon::Fire — 데드 코드 처리 + 위임** — A팀 코드의 `Fire()`가 `BlueprintCallable`로 노출돼 자체 LineTrace + ApplyDamage 별도 경로가 도는 이중 경로 발견. 진단 로그로 호출 0회 확인 후 본체 주석 처리 + `UWeaponComponent::TryFire` 위임 안전망만 유지
...
증상 — Project Settings에서 `BP_TestGameMode`로 바꿔도 PIE(Play In Editor, 에디터 안 테스트 실행) 진입 시 여전히 BP_NBC_GameMode가 떴다.
...
- 단순하고 직선적이지만 **디스크 I/O 동안 게임 스레드 블록** → 프레임 끊김. 무기 종류·블루프린트가 무거우면 체감 hitch(순간적으로 화면이 멈칫하는 끊김) 발생
...
> **오늘 배운 것** — `CustomTimeDilation`은 액터 단위라 좀비 Pawn만 멈추면 별개 액터인 AIController의 BT는 계속 돈다 — 히트스톱은 둘 다 멈춰야 완성된다. 그리고 모든 발사를 `TryFire` 한 곳으로 모아두면 새 효과를 진입점 한 군데에만 얹어도 모든 경로에 일관되게 적용된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "게임플레이 중 에셋을 동적으로 스폰할 때 동기 로드의 문제는 무엇이고 어떻게 해결하나요?" → 게임 스레드 블록·hitch, TSoftClassPtr, RequestAsyncLoad + 콜백, 멤버 변수로 컨텍스트 보관, IsValid 분기로 경로 통일
{: .prompt-info }
