...
1. **빌드 트러블슈팅 3건** — (a) `AActor::Instigator` shadowing → 파라미터명 `DeadInstigator`로 변경, (b) Figma2UMG 플러그인 미설치 → `.uproject`에 `"Optional": true` 추가 후 develop push, (c) `BaseWeapon` 헤더 `bIsOverHeat` vs cpp `bIsOverheated` typo 통일 → 빌드 통과 후 push
