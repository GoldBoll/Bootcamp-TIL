...
description: "정수 배열 nums를 오름차순으로 정렬하여 반환"
image: /assets/img/thumbs/leetcode.svg
---

> 출처: <https://leetcode.com/problems/sort-an-array/>

내장 정렬 함수 금지 + O(nlogn) 조건이라 `sort()` 한 줄로 끝낼 수 없는 문제다. 병합·힙·퀵 세 가지 정렬을 전부 직접 구현하면서 각각의 트레이드오프를 비교했다.
...
> **오늘 배운 것** — 같은 O(nlogn) 정렬이라도 성격이 다르다. Merge는 O(n) 버퍼가 필요하지만 안정 정렬(같은 값의 기존 순서 보존), Heap은 in-place(추가 배열 없이 제자리 정렬)지만 불안정, Quick은 평균이 가장 빠르지만 최악 O(n²)라 median-of-three 피벗으로 회피한다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "퀵 정렬이 평균적으로 가장 빠른데도 최악을 대비해야 하는 이유는?" → 최악 O(n²), median-of-three 피벗 선택, 재귀 스택 공간 O(logn), 안정 정렬 여부, 캐시 친화성
{: .prompt-info }
