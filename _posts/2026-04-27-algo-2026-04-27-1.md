...
image: /assets/img/thumbs/cards/2026-04-27-algo-2026-04-27-1.svg
---
...
> **핵심 요약** — 같은 O(nlogn) 정렬이라도 성격이 다르다. Merge는 O(n) 버퍼가 필요하지만 안정 정렬(같은 값의 기존 순서 보존), Heap은 in-place(추가 배열 없이 제자리 정렬)지만 불안정, Quick은 평균이 가장 빠르지만 최악 O(n²)라 median-of-three 피벗으로 회피한다.
{: .prompt-tip }
