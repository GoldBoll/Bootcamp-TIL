...
- **CS 면접 준비 (28편)** — 모의면접 질문 단위 정리. vtable·RTTI·스마트 포인터 같은 C++ 심화부터 프로세스/스레드·동기화·가상 메모리(OS), 자료구조까지. 각 글은 "30초 답변 → 꼬리질문 → 언리얼 연결" 구조입니다. 추천: [Race Condition](/posts/cs-23-race-condition/)
- **알고리즘 (80편+)** — 매일 코드카타. 프로그래머스·LeetCode·백준을 강사 코드 스타일(C++)로 풀고 "접근 → 코드 → 정리" 형식으로 남깁니다. 카드의 **"핵심 접근 —"** 한 줄로 어떤 기법인지 바로 확인할 수 있습니다.
- **TIL (70편+)** — 하루 단위 학습 기록. UE5 멀티플레이·툰 쉐이딩·머티리얼 작업이 중심입니다. 추천: [절차식 툰 스카이 머티리얼](/posts/til-toon-sky-material/), [BFS·시뮬레이션 시험 회고](/posts/til-bfs-simulation-exam/)
