...
description: "TObjectPtr·TSubclassOf 래핑 포인터와 TArray/TSet/TMap 컨테이너를 비교 정리하고, 인벤토리 시스템 필수 과제를 진행했다."
image: /assets/img/thumbs/til.svg
---

# 2026-04-17 언리얼 컨테이너와 래핑 포인터

오늘은 좋은 코드 이야기로 수업을 열고, UE5의 래핑 포인터(`TObjectPtr`, `TSubclassOf`)와 컨테이너 3종(`TArray` / `TSet` / `TMap`)을 배웠다. 이어서 7번 과제(Pawn 3D 캐릭터)의 구현 계획을 도전 과제 중심으로 다시 짰다.
...
긴 매개변수 목록, 전역 데이터, 가변 데이터 — 흔히 "코드 악취"라 부르는 세 가지를 언리얼 코드에 대입해서 살펴봤다.

### 긴 매개변수 목록 (Long Parameter List)

매개변수가 많으면 함수를 이해하고 쓰기가 불편하다. 필요한 정보만 전달되도록 묶거나 줄이고, 중복된 정보가 있는지 확인해서 불필요한 인수는 제거한다.
...
악취 중에서도 가장 독한 축에 든다. 어디서든 접근할 수 있으니 값이 언제 어디서 바뀌었는지 추적이 안 되고, 에러가 그 틈에 숨는다. 데이터의 범위를 최소화하고 꼭 필요한 곳에서만 쓰도록 통제해야 한다.
...
값이 자주 바뀌는 데이터는 예기치 못한 오류와 복잡도의 원인이 된다. 변경 가능한 범위를 최소화하고 가급적 불변 데이터를 쓰되, 수정이 필요한 부분은 명확히 나눠 두는 습관이 필요하다.
...
기존 "필수 → 도전" 분리 구조를 도전 우선 통합 플랜으로 다시 짰다. 그 과정에서 필수과제 요소 중 도전 구현과 충돌하는 것들을 버리기로 했다.
...
구현 계획을 다시 짜면서 헷갈렸던 것들을 정리했다.

- **6DOF 메서드 개수** — 축은 6개지만 메서드는 4개. `Move`·`Look`이 Axis2D라 한 함수가 2축을 담당하기 때문
...
- **`AddControllerYawInput/PitchInput` 금지 이유** — 과제 명세가 강제한 것. 로우레벨 원리 학습이 목표라서다
...
> **오늘 배운 것** — `TObjectPtr`은 성능 도구가 아니라 에디터용 관리 도구(지연 로딩·액세스 트래킹)이고, 패키징하면 원시 포인터로 바뀌어 기능도 함께 사라진다. 컨테이너는 탐색이 잦으면 `TSet`/`TMap`, 순회·인덱스 접근 위주면 `TArray`로 고른다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "TArray와 TSet/TMap 중 어떤 기준으로 선택하나요?" → 연속 메모리, 해시 테이블, 탐색 O(1) vs O(n), 순서 보장 여부, 중복 방지
{: .prompt-info }
