---
title: "프로그래머스 Lv.1 - 크기가 작은 부분문자열"
date: 2026-05-04 13:00:00 +0900
categories: ["알고리즘", "프로그래머스"]
tags: ["algorithm"]
render_with_liquid: false
description: "핵심 접근 — 길이 p인 부분문자열을 슬라이딩하며 수 비교."
image: /assets/img/thumbs/programmers.svg
---

> 출처: <https://school.programmers.co.kr/learn/courses/30/lessons/147355>

```cpp
// 프로그래머스 Lv.1 - 크기가 작은 부분문자열
// https://school.programmers.co.kr/learn/courses/30/lessons/147355
//
// [문제]  t의 부분문자열(길이=p) 중 나타내는 수가 p 이하인 것의 개수를 반환
// [제약]  p 길이 1~18 / p 길이 ≤ t 길이 ≤ 10,000 / 숫자 문자열, 0으로 시작 안 함
// [입출력]  "3141592","271"→2 / "500220839878","7"→8 / "10203","15"→3
//
// 풀이: 길이 p인 부분문자열을 슬라이딩하며 수 비교.
//        18자리까지 가능해 unsigned long long으로 변환 — O(n*|p|)

#include <string>
#include <vector>

using namespace std;

int solution(string t, string p) {
    int answer = 0;
    int lp = (int)p.size();
    unsigned long long pv = stoull(p);
    for (int i = 0; i + lp <= (int)t.size(); i++) {
        unsigned long long sub = stoull(t.substr(i, lp));
        if (sub <= pv) {
            answer++;
        }
    }
    return answer;
}
```

> **오늘 배운 것** — 부분문자열 길이가 p로 고정이라 한 칸씩 밀며 비교하면 끝나는 문제. 다만 p가 최대 18자리까지 가능해 int로는 넘치므로 `stoull`로 `unsigned long long` 변환 후 비교했다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "문자열로 주어진 큰 수를 비교할 때 무엇을 주의해야 하나?" → 자료형 범위 확인, 18자리, unsigned long long, stoull, 고정 길이 슬라이딩 비교
{: .prompt-info }

