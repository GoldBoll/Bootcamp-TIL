---
title: "툰 스카이 머티리얼 제작"
subtitle: "텍스처 0장, 수식만으로 그린 하늘"
date: 2026-07-06 12:30:00 +0900
categories: ["언리얼", "쿠펭"]
pin: true
tags: ["ue5", "material", "texture", "git", "트러블슈팅"]
render_with_liquid: false
description: "도시도 나무도 만화풍인데 하늘만 사실적인 볼류메트릭 구름이었고, 이 하늘은 포스트 프로세스 툰 셰이딩으로 잡히지 않는다. 그 이유와, 스카이돔을 대체한 8단계 노드 그래프."
image: /assets/img/posts/2026-07-06/sky-before.jpg
---

도시도 나무도 차량도 전부 만화풍인데 하늘만 사실적인 볼류메트릭 구름이었다. 그런데 이 하늘은 **포스트 프로세스 툰 셰이딩으로는 잡히지 않는다.** 이 글에서는 그 이유를 먼저 진단하고, 스카이돔 머티리얼을 **텍스처 0장짜리 절차식 툰 스카이**로 교체한 과정을 이야기하려 한다 — 구현은 여덟 단계 노드 그래프이고, 트러블슈팅은 이미 머지된 커밋에서 공동작업자 트레일러를 지우려다 빠진 **히스토리 재작성**이다.

## 요구사항

- 도시·나무·차량이 만화풍인데 하늘만 실사라 이질감이 크다 — 하늘을 툰 룩에 맞춘다
- 텍스처 의존 없이 시간대에 따라 색이 변할 수 있어야 한다
- 팀원이 작업 중인 레벨은 건드리지 않는다

## 과정

### 진단 — 하늘은 왜 포스트 프로세스로 못 잡나
...
- 셰이더 에셋은 개인 작업 폴더에 두고 작업한다. 이미 만든 걸 옮길 땐 리다이렉터가 남는지, 참조(돔 슬롯·MIC 부모)가 따라오는지 확인까지가 한 세트.
...
스카이돔 머티리얼을 텍스처 0장짜리 절차식 그래프로 교체해 도시와 하늘의 톤이 맞았다. 수식 기반이라 시간대 전환도 파라미터 보간으로 처리되고, 개인 작업 폴더의 레벨 사본에서 작업해 팀 레벨은 무변경으로 유지했다.

## 트러블슈팅 — 머지된 커밋의 트레일러는 지울 수 있는가
...
> **핵심 요약** — 하늘은 라이팅을 받는 표면이 아니라 배경이라서 포스트 프로세스 셀 셰이딩이 아니라 스카이돔의 Unlit 머티리얼 교체로 잡아야 하고, `saturate((x−t)/f)` 하나로 밴딩·구름·투톤을 전부 만들 수 있었다. git 쪽에서는 머지된 커밋의 재작성이 `commit-tree` plumbing으로 워킹 트리 없이 가능하다는 것과, force push는 팀 조율이 없으면 pull 한 번에 되돌아간다는 걸 몸으로 배웠다.
{: .prompt-tip }
