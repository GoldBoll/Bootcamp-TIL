...
description: "Ch3 히트 판정/데미지 시스템 디커플링 설계, UE5 모듈 중첩 Public/Private 함정, 코드 리뷰에서 채널 인덱스 미스매치 발견까지 정리."
...
6. **.gitignore + 커밋 위생** — `.idea/` / `*.DotSettings.user` / `Content/_Fab/` 추가. `.uproject AdditionalDependencies` 자동 주입은 커밋. Conventional Commits + 브랜치 전략으로 3개 커밋(chore / feat / chore _Fab)으로 분리
...
3개 커밋 분리 의도:
1. **chore (develop)** — 무기 작업 시작 전 ignore 정리. 이걸 무기 커밋과 섞으면 diff 가 지저분
2. **feat (브랜치)** — 무기 시스템 본 구현. 별도 브랜치(`feat/combat-hit-damage-system`)에서 작업해야 PR 흐름이 정상
3. **chore _Fab 확장 (develop)** — 무기 작업 도중 _Fab 패턴 보강(다른 변형 폴더 발견). develop에 직접 푸시해서 다른 브랜치들도 즉시 받게
