---
title: "1인 가구 들기 트러블슈팅"
subtitle: "서버와 클라가 같은 반경을 다른 자로 재고 있었다"
date: 2026-07-16 22:30:00 +0900
categories: ["언리얼", "쿠펭"]
tags: ["ue5", "cpp", "multiplayer", "network", "character-movement", "material", "debugging", "트러블슈팅"]
...
- 혼자 가구를 들고 이동할 때 서버와 클라이언트의 판정이 같아야 한다
- 카메라를 급하게 돌려도 가구가 몸을 통과하지 않아야 한다
...
측정 기준을 복제 트랜스폼으로 통일하자 앞서 넣었던 "클라 여유 반경 -8" 보정의 전제가 사라져 함께 걷어냈고, 리쉬 반경을 서버·클라 동일 값으로 단일화했다. 입력이 깎일 때만 발화하는 진단 로그를 붙여 재발 시 원인 항을 바로 판별할 수 있게 했다.
