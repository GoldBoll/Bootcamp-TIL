...
> **오늘 배운 것** — 문자열 마스킹은 "몇 번째부터 가릴지"를 인덱스 조건 하나(`i < size() - 4`)로 표현하면 반복문 한 번에 끝난다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "C++에서 문자열 일부를 다른 문자로 치환하는 방법을 설명해 보세요" → 인덱스 순회, 조건 분기, += 연결, substr 대안, O(N)
{: .prompt-info }
