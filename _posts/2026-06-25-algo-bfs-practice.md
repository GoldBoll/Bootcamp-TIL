...
> **오늘 배운 것** — BFS의 큐 골격은 그대로 두고 문제에 따라 누적값만 바꾸면 된다는 걸 4문제로 확인했다. 넓이는 cnt, 거리는 dist, 가중치 0/1은 덱 기반 0-1 BFS 또는 큐 2개 라운드 BFS로 가지를 친다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "가중치가 0과 1뿐인 그래프의 최단경로는 어떻게 구하는가?" → 0-1 BFS, 덱 사용, 비용 0은 push_front, 비용 1은 push_back, 다익스트라 없이 O(V+E)
{: .prompt-info }
