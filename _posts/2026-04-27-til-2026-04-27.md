---
title: "언리얼 아이템 데이터·스폰·픽업 파이프라인"
subtitle: "DataTable RowStruct는 왜 바꿀 수 없나"
date: 2026-04-27 09:00:00 +0900
categories: ["언리얼"]
tags: ["daily", "til", "트러블슈팅"]
render_with_liquid: false
description: "아이템 하나가 데이터로 정의되고 바닥에 스폰되고 주워지고, 그 무게가 이동 속도와 소음까지 바꾸는 경로를 한 줄로 연결했다. 도중에 DataTable과 DataAsset을 혼동해 막힌 두 건."
image: /assets/img/thumbs/cards/2026-04-27-til-2026-04-27.svg
---

아이템 하나가 게임에 들어오려면 거쳐야 하는 길이 길다. 데이터로 정의되고, 월드에 스폰되고, 플레이어가 줍고, 그 무게가 이동 속도와 소음에 반영되고, 마지막으로 손으로 상호작용할 수 있어야 한다. 이 글에서는 그 **아이템 파이프라인을 처음부터 끝까지 연결한 과정**을 이야기하려 한다. 구현은 스폰 위치 바닥 안착, 자동 픽업 흐름, 무게에 따른 감속과 소음 배율, Press E 인터랙션이고, 트러블슈팅은 그 앞을 막았던 **데이터 에셋 타입 문제 두 건** — DataTable의 RowStruct는 왜 바꿀 수 없는가, 그리고 이 자리가 DataTable이 아니라 DataAsset이었던 이유다.

## 트러블슈팅 1 — DataTable RowStruct 타입 미스매치
...
| 후보 | 검증 방법 | 결과 |
|---|---|---|
| C++ 구조체 빌드 누락 | `VOIDWaveData.h`에 `USTRUCT(BlueprintType)` + `GENERATED_BODY()` 확인 | 정상 |
...
## 트러블슈팅 2 — 이 자리는 DataTable이 아니라 DataAsset이었다
...
## 기술 구현 1 — SpawnVolume 라인 트레이스로 바닥 안착
...
`VOIDSpawnVolume.cpp`에 라인 트레이스 추가:
...
## 기술 구현 4 — Press E Sphere Trace 인터랙션
...
> **핵심 요약** — DataTable의 RowStruct는 생성 시 한 번 정해지면 사후 변경이 불가능해서, 잘못 만든 DT는 삭제·재생성뿐이다. 그리고 표 형식 데이터는 DataTable, 단일 객체 데이터는 DataAsset — 슬롯의 베이스 클래스를 보고 어느 쪽 자리인지 판단해야 같은 함정에 안 빠진다.
{: .prompt-tip }
