...
> **오늘 배운 것** — dp[i] = max(dp[i-1], dp[i-2] + nums[i]) 점화식이 직전 두 값에만 의존하므로, dp 배열 대신 롤링 변수 두 개로 공간을 O(n)에서 O(1)로 줄일 수 있다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "DP 문제에서 공간 복잡도를 줄이는 방법은?" → 점화식의 의존 범위, 롤링 변수, O(1) 공간, 상향식 반복
{: .prompt-info }
