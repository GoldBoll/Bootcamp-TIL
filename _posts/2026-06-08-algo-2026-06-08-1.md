...
> **오늘 배운 것** — "4개를 빼면 위아래가 다시 붙는다"는 연쇄 조건이, 스택에서는 pop 후 다음 push가 자연스럽게 이전 재료 위에 이어지는 성질 덕분에 별도 재배치 없이 처리된다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "이 문제에 왜 스택이 적합한가요?" → 제거 후 인접 재연결, top 4개만 검사, 재배치 불필요, O(n) 한 번 순회
{: .prompt-info }
