---
title: "2인 가구 들기 러버밴딩 트러블슈팅"
subtitle: "되끌어오는 견인을 리쉬 이동 제한으로 교체"
date: 2026-07-15 22:30:00 +0900
pin: true
categories: ["언리얼", "쿠펭"]
tags: ["ue5", "cpp", "multiplayer", "network", "character-movement", "debugging", "트러블슈팅"]
...
- 함께 든 두 사람이 대형을 벗어나지 않으면서 조작감이 끊기지 않아야 한다
- 서버 보정이 만들어 내는 되돌림(러버밴딩)이 없어야 한다

## 트러블슈팅 — 덜컹거리며 끌려오다 뚝 끊어진다
...
서버와 클라이언트가 같은 입력으로 같은 결론에 도달하면서 **교정 패킷 자체가 발생하지 않게** 됐다. 양쪽이 반대로 당겨도 가구는 중간에 고정되고 그랩은 유지되는 것을 치트로 재현해 확인했다.
