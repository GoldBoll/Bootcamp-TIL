---
title: "[TIL] 2026-03-17 — C++ 과제 1~3 정리"
date: 2026-03-17 09:00:00 +0900
categories: ["CS", "C++"]
tags: ["daily", "til"]
render_with_liquid: false
description: "C++ 과제 1~3 정리 — 상태창(기초 문법), 전직·전투 시스템(OOP), 템플릿·메모리 관리를 쓴 인벤토리까지 구현하고 언리얼과의 연결고리를 정리했다."
image: /assets/img/thumbs/til.svg
---

C++ 과제 1~3을 정리했다. 과제 1은 배열·포인터 기초, 과제 2는 상속·다형성, 과제 3은 템플릿과 메모리 관리로, 뒤로 갈수록 앞의 개념 위에 쌓이는 구성이었다.
...
> **오늘 배운 것** — 깊은 복사를 직접 구현하며 double free가 왜 생기는지 체감했고, 동적 배열 + 자동 확장 구조가 곧 `std::vector`와 `TArray`의 내부 원리라는 걸 연결해서 이해했다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "얕은 복사와 깊은 복사의 차이를 설명해 보세요." → 포인터 주소 복사, 같은 메모리 공유, double free, 복사 생성자에서 new 할당, vector/TArray 내부 원리
{: .prompt-info }
