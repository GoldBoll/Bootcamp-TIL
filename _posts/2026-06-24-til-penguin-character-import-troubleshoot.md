...
`unreal.Rotator`의 생성자 인자는 (Roll, Pitch, Yaw) 순이다. 보통 머릿속으로 "pitch부터"라고 생각하기 쉬워서 첫 인자에 pitch 값을 넣는 실수를 했다. pitch가 +45가 되면서 태양이 지평선 아래로 들어가 밤이 된 것.
...
원인 두 가지:
1. `checkf` 가드가 **이 빌드 구성에서 컴파일 제외**되어 null이 그대로 통과.
