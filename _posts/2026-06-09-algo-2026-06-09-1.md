...
> **오늘 배운 것** — 지표마다 두 유형의 점수를 따로 들지 않고 부호 있는 점수 하나로 압축하면, 동점 처리까지 `score >= 0` 비교 한 번으로 끝난다.
{: .prompt-tip }

> **면접에서 이렇게 말한다** — 예상 질문: "두 후보의 점수를 비교하는 로직을 어떻게 단순화할 수 있나요?" → 부호 있는 점수 하나로 압축, 사전순 정렬 배치, 동점 자동 처리, O(n) 한 번 순회
{: .prompt-info }
